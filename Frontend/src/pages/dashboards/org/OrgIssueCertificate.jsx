...
//   // Redux state
//   const { students, studentsLoading, studentsError } = useSelector(state => state.organization);
//   const { isLoading, isSuccess, isError, message } = useSelector(state => state.certificate);
...
//   // Reset certificate state on mount
//   useEffect(() => {
//     dispatch(resetCertificateState());
//   }, [dispatch]);
...
//       formData.append('description', certificateDesc);
...
//       await dispatch(issueCertificate(formData)).unwrap();
      
//     } catch (err) {
//       setError(err || 'Failed to issue certificate');
//     }
//   };
...
//       <div className="text-center">
//         <h1 className="text-2xl font-bold text-gray-900">Issue Certificate</h1>
//         <p className="text-gray-600 mt-2">Select a student and issue certificate</p>
//       </div>
...
//             <p className="text-green-700 text-sm">Certificate issued successfully</p>
//           </div>
//         </div>
//       )}
...
//       {/* Certificate Form - Only show when student is selected */}
//       {selectedStudent && (
//         <form onSubmit={handleSubmit} className="space-y-6">
//           {/* Certificate Details */}
//           <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
...
//             <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${
//               certificateFile ? 'border-green-300 bg-green-50' : 'border-gray-300 hover:border-gray-400'
//             }`}>
//               <input
//                 type="file"
//                 id="certificate-file"
...
//                   <FileText className="w-5 h-5 text-green-600" />
//                   <div>
//                     <p className="font-medium text-gray-900">{certificateFile.name}</p>
//                     <p className="text-sm text-gray-600">
...
//       {/* Click outside to close student list */}
//       {showStudentList && (
//         <div 
...
export default function OrgIssueCertificate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formData = new FormData();
      formData.append('studentId', selectedStudent._id); // ✅ Backend expects 'studentId'
      formData.append('title', certificateTitle);
      formData.append('description', certificateDesc);
      
      // ✅ IMPORTANT: File field name must be 'certificate' (matches multer config)
      formData.append('certificate', certificateFile);

      console.log("📤 Sending certificate data:", {
        studentId: selectedStudent._id,
        title: certificateTitle,
        fileField: 'certificate' // ✅ Confirming field name
      });
